...
def show_blobs(img: np.ndarray, peaks: np.ndarray, all_sigmas: np.ndarray, group_no: str = None, name: str = None) -> None:
    ...
    # student_code start
    # Flatten sigmas in case it's (levels, 1)
    sigmas_flat = np.array(all_sigmas).flatten()

    # Extract level indices from peaks and ensure they are integers
    level_indices = peaks[:, 2].astype(int)

    # Get σ for each peak using np.take
    sigmas_for_peaks = np.take(sigmas_flat, level_indices)

    # Compute radii using r = sqrt(2) * σ
    radi = np.sqrt(2.0) * sigmas_for_peaks
    # student_code end
